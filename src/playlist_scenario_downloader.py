...
class KovaaksScenarioManager:
    ...
    def load_config(self):
        default_paths = {
            ...
            "logs_dir": os.path.join(
                os.path.expanduser("~"), "Documents", "Kovaaks Logs"
            ),
        }
    ...
    def open_path_settings(self):
        ...
        for i, (label, config_key) in enumerate(paths):
            tk.Label(path_dialog, text=label).grid(
            ...
            browse_btn = tk.Button(
                path_dialog, text="Browse", command=create_browse_command(entry)
            )
    ...
    def terminate_process(self, script_name="playlist_scenario_downloader.py"):
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            if proc.info["name"] == "python.exe" and script_name in " ".join(
                proc.info["cmdline"]
            ):
                Kovaaks_Tool.Main_Menu()
                proc.terminate()  # or proc.kill() to force kill
                print(
                    f"Terminated Python script: {script_name} (PID: {proc.info['pid']})"
                )
                return
    ...
    def create_widgets(self):
        ...
        settings_menu.add_command(
            label="Path Settings", command=self.open_path_settings
        )
        main_frame = tk.Frame(self.master)
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        left_frame = tk.Frame(main_frame)
        left_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 5))
        tk.Label(left_frame, text="Playlists", font=("Arial", 12, "bold")).pack()
        ...
        search_btn = tk.Button(
            search_frame, text="Search Playlists", command=self.search_playlists
        )
        search_btn.pack(side=tk.LEFT)
        self.playlist_tree = ttk.Treeview(
            left_frame, columns=("Name", "Scenarios"), show="headings"
        )
        ...
        tk.Label(right_frame, text="Scenarios", font=("Arial", 12, "bold")).pack()
        self.scenario_tree = ttk.Treeview(
            right_frame, columns=("Scenario", "Exists"), show="headings"
        )
        ...
        tk.Button(
            button_frame,
            text="Move Scenarios",
            command=self.move_scenarios_from_workshop,
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            button_frame, text="Download Selected", command=self.download_selected
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            button_frame, text="Back to Menu", command=lambda: self.terminate_process()
        ...
        def fetch_thread():
            try:
                ...
                if search_term:
                    ...
                    playlists_url += (
                        f"&search={urllib.parse.quote(sanitized_search_term)}"
                    )
                ...
                for playlist in self.playlists_data:
                    if not playlist:
                        continue
                    playlist_name = self.sanitize_text(
                        playlist.get("playlistName", "Unknown")
                    )
                    try:
                    ...
                    if playlist_name:
                        self.playlist_tree.insert(
                            "",
                            "end",
                            values=(playlist_name, scenario_count),
                            tags=(playlist_name,),
                        )
                self.log(
                    f"Fetched {len(self.playlists_data)} playlists"
    ...
    def on_playlist_select(self, event=None):
        ...
        for playlist in self.playlists_data:
            if playlist is None:
                continue
            playlist_name_check = self.sanitize_text(
                playlist.get("playlistName", "Unknown")
            )
            ...
            if playlist_name_check == playlist_name:
                ...
                for scenario in scenarios:
                    scenario_name = self.sanitize_text(
                        scenario.get("scenarioName", "Unknown")
                    )
                    in_workshop = any(
                        scenario_name in f
                        for f in os.listdir(self.workshop_dir)
                        ...
                        if file.endswith(".sce"):
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    self.log(
                                        f"Webhook error for {file}: {webhook_error}"
                                    )
    ...
    def create_download_buttons(self):
        ...
        def download_selected_thread():
            ...
            if selected_playlist_item:
                selected_playlist = self.playlist_tree.item(selected_playlist_item)[
                    "values"
                ][0]
            ...
            for link in scenario_links:
                try:
                    ...
                    subprocess.run(
                        'taskkill /F /IM "FPSAimTrainer-Win64-Shipping.exe"', shell=True
                    )
                    time.sleep(0.5)
        ...
        def download_all_new_thread():
            ...
            for playlist in self.playlists_data:
                if (
                    playlist
                    and self.sanitize_text(playlist.get("playlistName", "Unknown"))
                    == playlist_name
                ):
                    all_scenarios = [
                        ...
                        if scenario
                        and self.sanitize_text(scenario.get("scenarioName", "Unknown"))
                        not in self.existing_scenarios
                    ]
                    scenario_links = [
                        f"steam://run/824270/?action=jump-to-scenario;name={urllib.parse.quote(scenario)}"
        ...
        selected_btn = tk.Button(
            ...
            command=lambda: threading.Thread(
                target=download_selected_thread, daemon=True
            ).start(),
        )
        ...
        all_btn = tk.Button(
            ...
            command=lambda: threading.Thread(
                target=download_all_new_thread, daemon=True
            ).start(),
        )
    ...
    def download_selected(self):
        if not self.playlist_tree.selection():
            messagebox.showwarning(
                "No Playlist Selected", "Please select a playlist first."
            )
            return

        if len(self.scenario_tree.get_children()) == 0:
            messagebox.showwarning(
                "No Scenarios", "No scenarios found in the selected playlist."
            )
            return
