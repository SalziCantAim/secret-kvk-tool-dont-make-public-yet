...
class AutoClipper:
    ...
    def connect_obs(self, host="localhost", port=4455, password=""):
        ...
        try:
            ...
            messagebox.showerror("OBS Connection Error", f"Failed to connect to OBS: {str(e)}")
        ...
        try:
            ...
            if self.obs_client:
                try:
                    ...
                    messagebox.showerror("Replay Buffer Error", f"Failed to start Replay Buffer: {str(e)}")
                    return False
                return True
                ...
                try:
                    ...
                    try:
                        self.obs_client.trigger_media_input_action(
                            input_name="ReplayBuffer",
                            action_name="ReplayBuffer.Save"
                        )
...
class ScoreEventHandler(FileSystemEventHandler):
    ...
    def process_score_file(self, file_path):
        ...
        is_new_high_score, scenario, score = self.score_tracker.process_new_score_file(file_path)
...
class ObsWindow:
    ...
    def connect_obs(self, root):
        ...
        if self.clipper.connect_obs(
            ...
            messagebox.showinfo("Success", "Connected to OBS and started Replay Buffer!")
            clear_window()
...
def save_obs_replay(scenario, score):
    ...
    try:
        ...
        if not mp4_files:
            ...
            return None
        latest_file = max(
            (os.path.join(clip_path, f) for f in mp4_files),
...
def get_basic_ini_path():
    ...
    if os.path.exists(location_file):
        ...
        basic_ini_path = filedialog.askopenfilename(
            ...
            initialdir=os.path.expanduser(rf"C:/Users/{user}/AppData/Roaming/obs-studio/basic/profiles")
        )
        ...
        if basic_ini_path:
            ...
            messagebox.showwarning("No basic.ini File", "Please select a valid basic.ini file.")
            return None
...
def get_rec_file_path():
    ...
    try:
        ...
        if match:
            ...
            messagebox.showwarning("Not Found", "RecFilePath not found in the basic.ini file.")
            return None
...
def terminate_process(script_name="Autoclipping.py", root_argument="root"):
    ...
    clear_window()
    try:
        cmd_list_processes = 'wmic process where "Name like \'%python.exe%\' and CommandLine like \'%{}%\' and CommandLine like \'%{}%\'" get ProcessId /value'.format(script_name, root_argument)
        result = subprocess.run(cmd_list_processes, stdout=subprocess.PIPE, text=True, shell=True)
        ...
        for line in output.splitlines():
            ...
            if "ProcessId=" in line:
                ...
                print(f"Terminated process: {script_name} with argument '{root_argument}' (PID: {pid})")
                return
        print(f"No matching process found for {script_name} with argument '{root_argument}'.")
    ...
    try:
        ...
        if not os.path.exists("custom_config.json"):
            ...
            if any(proc.name() == "obs64.exe" for proc in psutil.process_iter()):
                messagebox.showerror("Restart OBS", f"Please restart OBS to use the Auto Clipping function!")
            else:
...
if __name__ == "__main__":
    ...
    for file_name in os.listdir(stats_dir):
        ...
        if file_name.endswith('.csv') and file_name not in score_tracker.processed_files:
