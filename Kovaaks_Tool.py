...
def change_scenario_folder():
    ...
    if Folder_directory:
        ...
        messagebox.showwarning("No Scenarios", "Please select a valid Scenarios folder.")
        clear_window()
        Main_Menu()
        return None
    return Folder_directory
...
def AutoClipperGUI(colors):
    ...
    try:
        subprocess.run(
            ["python", "Autoclipping.py", "root", json.dumps(colors)],
            check=True
        )
...
def Profile_Changer():
    ...
    def open_Profile_Changer(scenario):
        ...
        print(scenario)
        subprocess.run(["python", "ProfileChangeHandlerTest.py", main_menu, scenario], check=True)
    ...
    def list_scenarios():
        ...
        def update_scenario_list(search_text=""):
            ...
            for widget in scrollable_frame.winfo_children():
                widget.destroy()
            filtered_files = [f for f in all_scenario_files if search_text.lower() in f.lower()]
            ...
            for index, scenario in enumerate(filtered_files):
                ...
                button.pack(fill="x")
        search_entry.bind("<KeyRelease>", lambda event: update_scenario_list(search_entry.get()))
...
def create_playlist():
    ...
    def get_scenario_directory():
        ...
        if os.path.exists(location_file):
            ...
            if scenario_directory:
                ...
                messagebox.showwarning("No Folder Selected", "Please select a valid scenario folder.")
                return None
    ...
    def get_Playlist_directory():
        ...
        if os.path.exists(location_file):
            ...
            if Folder_directory:
                ...
                messagebox.showwarning("No Playlist", "Please select a valid Playlist folder.")
                return None
        return Folder_directory
    selected_scenarios = {}
    filtered_scenarios = []
    ...
    def update_chosen_list(frame):
        ...
        for scenario, details in selected_scenarios.items():
            ...
            button_frame = tk.Frame(row_frame, bg="white", highlightbackground="black", highlightthickness=1)
    ...
    def add_to_playlist(scenario_name, chosen_frame):
        ...
        if scenario_name in selected_scenarios:
            ...
            selected_scenarios[scenario_name] = {"scenario_name": scenario_name, "play_Count": 1}
    ...
    def search_scenarios(entry, canvas, frame, scenarios, chosen_frame):
        ...
        filtered_scenarios = [scenario for scenario in scenarios if search_term in scenario.lower()]
    ...
    def populate_scenario_buttons(canvas, frame, scenarios, chosen_frame):
        ...
        for index, scenario in enumerate(scenarios):
            ...
            row_color_flag = (row % 2 == 0)
            bg_color = BACKGROUND_COLOR if row_color_flag else SECONDARY_COLOR
            button_frame = tk.Frame(frame, bg="white", highlightbackground="black", highlightthickness=1)
            button_frame.grid(row=row, column=col, padx=5, pady=5, sticky="nsew")
            button = tk.Button(button_frame, text=scenario, command=lambda s=scenario: add_to_playlist(s, chosen_frame), font=("Helvetica", 11, "bold"),
                               width=40, bg=bg_color, fg=TEXT_COLOR, activebackground=bg_color, relief="flat")
    ...
    def create_input_section(parent_frame, title, prompt, on_submit, back_button=False, on_back=None):
        ...
        input_entry = tk.Entry(parent_frame, font=("Helvetica", 12), bg="white", fg="black")
    ...
    def save_playlist():
        ...
        if not selected_scenarios:
            create_input_section(right_frame, "No Scenarios", "No scenarios selected for the playlist.", lambda _: None)
            return
        ...
        def get_playlist_name(playlist_name):
            ...
            if not playlist_name:
                create_input_section(right_frame, "Input Error", "Please provide a valid playlist name.",
                                     lambda _: None)
                return
            ...
            def get_description(description):
                ...
                scenario_list = [
                    {"scenario_name": details['scenario_name'].replace('.sce', ''), "play_Count": details['play_Count']}
                ...
                create_input_section(right_frame, "Success", f"Playlist '{playlist_path}' saved successfully!",
                                     lambda _: None)
            create_input_section(right_frame, "Description", "You can add a description:", get_description)
        create_input_section(right_frame, "Playlist Name", "Enter the playlist name:", get_playlist_name)
    ...
    def create_playlist_window(scenarios):
        ...
        left_frame.pack(side=tk.LEFT, padx=0, pady=3, anchor="nw", fill=tk.BOTH, expand=True)
        ...
        chosen_label = tk.Label(right_frame, text="Chosen Scenarios", font=("Helvetica", 15, "bold"), bg=header_color,
                                fg=TEXT_COLOR)
        chosen_label.pack(padx=(0, 0), pady=23)
        chosen_canvas = tk.Canvas(right_frame, bg=BACKGROUND_COLOR, highlightthickness=0, bd=0)
        ...
        save_button = tk.Button(top_frame, text="Save Playlist", command=save_playlist, bg=PRIMARY_COLOR,
                                font=("Helvetica", 15, "bold"), fg=TEXT_COLOR, relief="flat")
        save_button.pack(pady=8, padx=(430, 0))
        chosen_frame = tk.Frame(chosen_canvas, bg=BACKGROUND_COLOR, highlightthickness=0, bd=0)
        chosen_canvas.create_window((0, 50), window=chosen_frame, anchor="nw")
        search_label = tk.Label(left_frame, text="Search Scenarios:", bg=header_color, font=("Helvetica", 15, "bold"),
                                fg=TEXT_COLOR)
        ...
        scrollbar = tk.Scrollbar(left_frame, orient=tk.VERTICAL, command=canvas.yview, width=0)
        ...
        back_button.place(relx=1.0, rely=1.0, anchor="se", x=-10, y=-10)
        scenario_frame = tk.Frame(canvas, bg=BACKGROUND_COLOR, highlightthickness=0, bd=0)
        ...
        canvas.config(scrollregion=canvas.bbox("all"))
        selected_scenarios = {scenario: {"scenario_name": scenario, "play_Count": 0} for scenario in scenarios}
        ...
        def adjust_left_frame():
            ...
            required_width = (button_width * 2) + gap_between_buttons + scrollbar_width + 10
            canvas.config(width=required_width)
            left_frame.config(width=required_width)
            left_frame.config(height=root.winfo_height())
            canvas.config(height=root.winfo_height() - search_entry.winfo_height() - search_label.winfo_height())
        adjust_left_frame()
        search_entry.bind("<KeyRelease>", lambda event: search_scenarios(
